...
bool test9(std::string& error){
    ...
    SortedList<Record>::iterator front;
    SortedList<Record>::iterator back;
    front=theList.begin();
    back=theList.end();
    for(int i=0;i<17;i++){
        front++;
    }
    result=theList.erase(front,back);
    if(result!=theList.end()){
        rc=false;
        error = "Error 9a: The return value from erasing last 3 nodes in list should be end()";
    }
    if(theList.size()!=17){
        rc=false;
        error = "Error 9b: The size() is wrong after calling erase(front,back) on last 3 nodes in list\n";
    ...
    for(int i=17;rc && i<20;i++){
        if(theList.search(sortedRecs[i]) != theList.end()){
            error = "Error 9c: after erase(front,back) on last 3 nodes in list, item that was erased was found but should not have been";
            rc=false;
        }
    }
    for(int i=0;rc && i<17;i++){
        if(theList.search(sortedRecs[i]) == theList.end()){
            error = "Error 9d: a node that should be in the list isn't after using erase(front,back) on last 3 nodes in list";
            rc=false;
        }
    }
    if(rc){
        ...
        if(result!=back){
            rc=false;
            error = "Error 9e: The return value from erasing first 3 nodes was not correct";
        }
        if(theList.size()!=14){
            rc=false;
            error = "Error 9f: The size() is wrong after calling erase(front,back) on first 3 nodes in list\n";
            error = "your function returned: ";
            error += std::to_string(theList.size());
        ...
        for(int i=0;rc && i<3;i++){
            if(theList.search(sortedRecs[i]) != theList.end()){
                error = "Error 9g: after erase(front,back) on first 3 nodes in list, item that was erased was found but should not have been";
                rc=false;
            }
        }
        for(int i=3;rc && i<14;i++){
            if(theList.search(sortedRecs[i]) == theList.end()){
                error = "Error 9h: a node that should be in the list isn't after using erase(front,back) on first 3 nodes in list";
                rc=false;
            }
        }        
    }
    ...
    if(rc){
        ...
        if(result!=back){
            rc=false;
            error = "Error 9i: The return value from erasing(front,back) was not correct";
        }
        if(theList.size()!=6){
            rc=false;
            error = "Error 9j: The size() is wrong after calling erase(front,back)\n";
            error += "your function returned: ";
            error += std::to_string(theList.size());
        ...
        for(int i=3;rc && i<6;i++){
            if(theList.search(sortedRecs[i]) == theList.end()){
                error = "Error 9k: a node that should be in the list isn't after using erase(front,back)";
                rc=false;
            }
        }
        for(int i=14;rc && i<17;i++){
            if(theList.search(sortedRecs[i]) == theList.end()){
                error = "Error 9l: a node that should be in the list isn't after using erase(front,back)";
                rc=false;
                std::cout << i << std::endl;
            }
        }
        for(int i=6;rc && i<14;i++){
            if(theList.search(sortedRecs[i]) != theList.end()){
                error = "Error 9m: after erase(front,back), item that was erased was found but should not have been";
                rc=false;
            }
        }        
    }
